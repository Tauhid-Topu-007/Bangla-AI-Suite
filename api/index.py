...
async def code_documentation(input_data: TextInput):
    ...
    if 'def ' in code:
        lang = "পাইথন"
        doc = f'"""\nফাংশনের ডকুমেন্টেশন\n\nএই ফাংশনটি নির্দিষ্ট কাজ সম্পাদন করে।\n"""'
    else:
        lang = "অজানা"
        doc = f"/*\nকোড ডকুমেন্টেশন\n\n{code[:50]}...\n*/"
...
async def resume_parser(input_data: ResumeInput):
    ...
    return {"নাম": input_data.name, "ইমেইল": input_data.email, "দক্ষতা": skills_list, "এটিএস স্কোর": f"{job_match}%"}
...
<body>
    ...
    <div class="container">
        ...
        <div class="main-panel">
            <div class="panel-title" id="active-name">📝 টেক্সট সামারাইজার</div>
    ...
    <script>
        ...
        function updateLiveTime() {
            ...
            document.getElementById('time').innerHTML = `${formattedHours}:${minutes}:${seconds} ${ampm}`;
            
            if (hours < 12) document.getElementById('greeting').innerHTML = 'শুভ সকাল';
            else if (hours < 18) document.getElementById('greeting').innerHTML = 'শুভ বিকাল';
            else document.getElementById('greeting').innerHTML = 'শুভ রাত্রি';
        }
        ...
        function renderTools() {
            ...
            for (let i = 0; i < tools.length; i++) {
                ...
                grid.innerHTML += `<div class="tool-card ${activeClass}" onclick="selectTool('${t.id}')"><div class="tool-icon">${t.icon}</div><div class="tool-name">${t.name}</div></div>`;
            }
        }
        ...
        function selectTool(toolId) {
            ...
            renderTools();
            document.getElementById('active-name').innerHTML = `${currentTool.icon} ${currentTool.name}`;
        ...
        async function processText() {
            ...
            try {
                ...
                if (toolId === 'email-response') {
                    body = { email_content: text || 'আমি জানতে চাই', tone: document.getElementById('email-tone')?.value || 'professional' };
                    ...
                    body = {
                        ...
                        skills: document.getElementById('resume-skills')?.value || 'পাইথন',
                        experience: document.getElementById('resume-experience')?.value || '৫ বছর',
                        education: document.getElementById('resume-education')?.value || 'বিএসসি'
                    };
                } else if (toolId === 'interview-prep') {
                    body = {
                        question: document.getElementById('interview-question')?.value || 'পরিচয় দিন',
                        answer: document.getElementById('interview-answer')?.value || text || 'আমি দক্ষ'
                    };
                } else {
